...
from .utils import try_int, decimal_or_zero, try_str, int_or_same, int_or_zero, int_or_empty, agregar_codigo_error
...
class De(MysqlConnection.Base):
    ...
    de_id = Column(String(100), nullable=True)
        ...
        if REUTILIZAR_CDC:
            ...
            if documento_previo:
                if (str(self.iTiDE) != str(documento_previo.iTiDE)
                    ...
                    self.dCodSeg = str(documento_previo.dCodSeg).zfill(9)
    ...
    def generar_cdc(self):
        ...
        for k, v in dict_id_fields.items():
            if not v and v != 0:
                ...
                self.x_error += agregar_codigo_error(f"ERROR en getId(): No puede estar vacio el campo: {k} ")
        ...
        if (v_len_cdc != 44):
            ...
            self.x_estado = self.ERROR_PREVALIDACION_INTERNA
            self.x_error += agregar_codigo_error(f"ERROR en getId(): La longitud del CDC debe ser de 44 caracteres y " \
    ...
    def getVDigit(self, numero):
        if isEmpty(numero):
            ...
            self.x_estado = self.ERROR_PREVALIDACION_INTERNA
            self.x_error += agregar_codigo_error(f"ERROR en getVDigit(): No se puede obtener el digito verificador sin tener el valor del numero")
    ...
    def getdCodSeg(self):
        ...
        v_num_valid = False
        v_rand = 0
        while not v_num_valid:
            v_rand = random.randrange(1, 999999999)
            if try_int(self.dNumDoc) != try_int(v_rand):
                v_num_valid = True
    ...
    def getdDesTiDE(self):
        ...
        if not self.iTiDE:
            ...
            self.x_estado = self.ERROR_PREVALIDACION_INTERNA
            self.x_error += agregar_codigo_error(f"ERROR en getdDesTiDE(): No puede estar vacio el campo iTiDE " \
                f"(Tipo de Documento Electrónico) para obtener su descripcion")
        else:
    ...
    def getdPlazoCre(self):
        ...
        if self.iCondCred == 1:
            ...
            self.x_estado = self.ERROR_PREVALIDACION_INTERNA
            self.x_error += agregar_codigo_error(f"ERROR en getdPlazoCre(): Solamente es obligatorio si E641(iCondCred) = 1")
    ...
    def obtener_valores_calculados(self, session):
        ...
        try:
            ...
            self._Id = "0" * 44
            raise ValueError(f"Error al generar el cdc, verifique los datos enviados: {error}")
    ...
    def validar_reglas_negocio(self):
        ...
        if self.dRucRec:            
            ...
            if self.dRucRec.isnumeric():
                ...
                if str(digito_verificador) != str(self.dDVRec):
                    mensaje_error = agregar_codigo_error(
                        f"ERROR dRucRec: El dígito verificador del RUC del Receptor ({self.dRucRec}) "
                        f"debería ser {digito_verificador} en lugar de '{self.dDVRec}'")
                    return False, mensaje_error
        ...
        if (self.iTiDE == 4 or self.iTiDE == 5 or self.iTiDE == 6) and not self.documentos:
            mensaje_error = agregar_codigo_error(f"ERROR gCamDEAsoc: Si es Autofactura, NC o ND debe tener documentos asociados")
            return False, mensaje_error
    ...
    def generarXML(self):
        ...
        for row in csvData:
            ...
            # No informar el tag dTotalGs si cMoneOpe == "PYG"
            if self.cMoneOpe == "PYG" and (row[0].strip() == "dTotalGs"):                
                continue
                    ...
                    if row[0].strip() != "Id":
                        if row[2].strip() == "0":
                            ...
                            excepciones = ("dDVId", "dNumCas","dTotOpe", "dTotDesc","dTotDescGlotem", "dTotAntItem",
                                           ...
                                           "dSalAnt", "dTBasGraIVA", "dSubExe", "dSub5", "dMonTiPag", "dBasExe", "dSubExo", 
                                           "dTotalGs")                                           
        ...
        for action, elem in context:
            ...
            if self.recursively_empty(elem):
                if elem.tag not in ("dDVId", "dNumCas","dTotOpe", "dTotDesc","dTotDescGlotem", "dTotAntItem", "dTotAnt",
                                    ...
                                    "dMonTiPag", "dBasExe", "dSubExo", "dTotalGs"):
                    parent.remove(elem)
    ...
    def validar_xml_xsd(self, xmlstr):
        ...
        if isvalid and not self.x_estado == self.ERROR_PREVALIDACION_INTERNA:
            ...
            self.x_estado = self.ERROR_PREVALIDACION_INTERNA
            self.x_error += agregar_codigo_error(err_msj)
    ...
    def ejecutar_proceso_completo(self, session, proceso, OUTPUT_PATH, tipo_servicio='A', cdc_docs_procesados=[]):
        try:
            ...
            try:
                ...
                self.x_estado = self.ERROR_PREVALIDACION_INTERNA
                self.x_error += agregar_codigo_error(str(error))

            reglas_ok, msg = self.validar_reglas_negocio()

            if not reglas_ok:
                self.x_estado = self.ERROR_PREVALIDACION_INTERNA
                self.x_error += msg #ya sa valida el agregar_codigo_error en la funcion validar_reglas_negocio!

            is_duplicated = False
            if self._Id in cdc_docs_procesados:
                is_duplicated = True
                self.x_estado = self.ERROR_PREVALIDACION_INTERNA
                self.x_error += " A002:CDC Duplicado en el lote enviado "
                # Cambiamos el cdc a 0 porque puede causar muchos problemas
                # self._Id = "0" * 44
            ...
            self.x_error += agregar_codigo_error(f" Error no controlado: {ex}")
    ...
    def get_country(self):
        if self.cPaisRec:
            ...
            print(err_msg)
            self.x_estado = De.ERROR_PREVALIDACION_INTERNA
            self.x_error += agregar_codigo_error(err_msg)
            return ""
...
class Item(MysqlConnection.Base):
    ...
    dBasExe = Column(Numeric(23, 8), nullable=True)
    ...
    def get_dBasExe(self):
        """        
        Si iAfecIVA = 4 este campo es igual al resultado del cálculo:
        ...
        """
        if self.iAfecIVA == 4:
            self.dBasExe = round((100 * self.dTotOpeItem * (100 - self.dPropIVA)) / (10000 + (self.dTasaIVA * self.dPropIVA)), 8)
        elif self.iAfecIVA == 1 or self.iAfecIVA == 2 or self.iAfecIVA == 3:
            self.dBasExe = 0
        else:
            print("Valor iAfecIVA no reconocido para calcular dBasGravIVA")
