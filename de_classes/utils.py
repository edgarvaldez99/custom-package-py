...
def leer_codigos_error():
    this_folder = os.path.dirname(os.path.abspath(__file__))
    my_file = os.path.join(this_folder, 'codigos_error.csv')
    with open(my_file, mode='r', encoding='utf8') as csv_file:
        csv_reader = csv.DictReader(csv_file, delimiter=';')
        return list(csv_reader)


list_codigos_error = leer_codigos_error()


def agregar_codigo_error(texto):
    """
    Recibe como parametro el texto, del listado de codigo de error, 
    busca coincidencia y retorna el código de error.
    """
    # Procesar los datos utilizando el diccionario en caché
    if texto == '': 
        return texto
    for item in list_codigos_error:
        etiqueta = item['etiqueta']
        codigo = item['codigo']
        if etiqueta.strip() in texto:
            return f'{codigo}:{texto} '
    
    # si la palabra no se encuentra, retornar un codigo genérico
    return f"A001: {texto} "
